...
function animate()
{
    man.forEach(t =>
    {
        ...
        if (t.position.y > parachute_jump.value)
        {
            ...
            // //سقوط الحر
            data['free_acc_y'] = free_acc_y.toFixed(4);
            free_acc_y = gravity / 120;

            data['free_velocity_y'] = free_velocity_y.toFixed(4);
        ...
        if (t.position.y <= parachute_jump.value && t.position.y >= 0)
        {
            ...
            data['variant'] = variant.toFixed(4);
            variant += ((rope_force * rope.length) / (rope.area * (Young.factor * 1000))) / 120;

            let length_range = rope.length / 6;
            data['length range'] = length_range.toFixed(4);

            let longest = rope.length + length_range;
            data['longest length'] = longest.toFixed(4);

            let current_rope_now = rope.length + variant / 60;
            data['Current rope'] = current_rope_now.toFixed(4);
